...
from apps.knowledge_manager.src.graphrag.index.utils.json import clean_up_json
...
from apps.knowledge_manager.src.graphrag.query.llm.base import BaseLLM
...
from apps.knowledge_manager.src.graphrag.query.structured_search.global_search.callbacks import (
...
class GlobalSearch(BaseSearch):
    ...
    async def asearch(
        ...
        **kwargs: Any,
        ...
        if run_reduce_response:
            ...
            global_search_response = reduce_response.response
            global_search_reduce_data = reduce_response.context_data
            global_search_reduce_text = reduce_response.context_text
            global_search_reduce_llm_calls = reduce_response.llm_calls
            global_search_reduce_prompt_tokens = reduce_response.prompt_tokens
        else:
            reduce_context_text = await self._generate_reduce_context_data(
                map_responses=map_responses
            )
            global_search_response = reduce_context_text
            global_search_reduce_data = reduce_context_text
            global_search_reduce_text = reduce_context_text
            global_search_reduce_llm_calls = 0
            global_search_reduce_prompt_tokens = 0

        # print(f"[1:Step2] {time.time() - st}")

        return GlobalSearchResult(
            response=global_search_response,
            context_data=context_records,
            context_text=context_chunks,
            map_responses=map_responses,
            reduce_context_data=global_search_reduce_data,
            reduce_context_text=global_search_reduce_text,
            completion_time=time.time() - start_time,
            llm_calls=map_llm_calls + global_search_reduce_llm_calls,
            prompt_tokens=map_prompt_tokens + global_search_reduce_prompt_tokens,
            resources=None,
        )
    ...
    async def _generate_reduce_context_data(
        self,
        map_responses: list[SearchResult],
    ...
    async def _reduce_response(
        ...
        **llm_kwargs,
        ...
        start_time = time.time()
        try:
            text_data = await self._generate_reduce_context_data(
                map_responses=map_responses
            )
            if text_data == NO_DATA_ANSWER:
                return SearchResult(
            ...
            text_data = ""
            log.exception(f"Exception in reduce_response: <{e}>\n{exc}")
            return SearchResult(
                response="",
                context_data=text_data,
                context_text=text_data,
