...
from apps.knowledge_manager.src.graphrag.model import Covariate, Entity, Relationship
from apps.knowledge_manager.src.graphrag.query.input.retrieval.covariates import (
...
from apps.knowledge_manager.src.graphrag.query.input.retrieval.relationships import (
...
def build_entity_context(
    ...
    # add headers
    current_context_text = f"<dataset name> = -----{context_name}-----" + "\n"
    header = [
        "id",
        "entity",
        "description",
    ...
    if len(all_context_records) > 1:
        all_context_records[0].insert(3, "id_full")
        for ii in range(len(all_context_records[1:])):
            found = False
            for se in selected_entities:
                if se.short_id == all_context_records[ii + 1][0]:
                    all_context_records[ii + 1].insert(3, se.id)
                    found = True
                    break

            if not found:
                all_context_records[ii + 1].insert(3, None)
...
def build_relationship_context(
    ...
    if len(all_context_records) > 1:
        all_context_records[0].insert(4, "id_full")
        for ii in range(len(all_context_records[1:])):
            found = False
            for se in selected_relationships:
                if se.short_id == all_context_records[ii + 1][0]:
                    all_context_records[ii + 1].insert(4, se.id)
                    found = True
                    break

            if not found:
                all_context_records[ii + 1].insert(4, None)
