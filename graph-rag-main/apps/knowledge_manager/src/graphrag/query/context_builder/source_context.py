...
from apps.knowledge_manager.src.graphrag.model import Entity, Relationship, TextUnit
...
def center_slice(s: str, N_LEN_CHAR: int = 300) -> str:
    if N_LEN_CHAR >= len(s):
        # If the requested length is greater than or equal to the string length, return the entire string
        return s

    # Calculate the start index
    start = (len(s) - N_LEN_CHAR) // 2
    # Calculate the end index
    end = start + N_LEN_CHAR
...
def build_text_unit_context(
    ...
    for unit in text_units:
        new_context = [
            unit.short_id,
            center_slice(unit.text),  # TODO: CHANGE LOGIC, HARD CODE
            *[
