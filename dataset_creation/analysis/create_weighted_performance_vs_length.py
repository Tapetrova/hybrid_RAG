    ...
    if qid in question_lengths:
        ...
        for min_len, max_len, bin_label in length_bins:
            if min_len <= q_length < max_len:
                # Calculate WFAS for each method using 2:1 weight scheme
                for method in methods:
                    if method in result.get('metrics', {}):
                        ...
                        if metrics.get('total_claims', 0) > 0:
                            ...
                            # WFAS = 100 - (2*contradicted + 1*unverifiable)/(3*total) * 100
                            whr = ((2 * contradicted + 1 * unverifiable) / (3 * total)) * 100
                            wfas = 100 - whr
...
ax.set_ylabel('Weighted Factual Accuracy Score (%)', fontsize=18, fontweight='bold')
ax.set_title('Performance vs Question Length\n(Weighted metrics: 2× penalty for contradictions)', 
...
ax.set_yticklabels([f'{i}%' for i in range(50, 101, 10)], fontsize=14)
...
print("\n" + "="*50)
print("WEIGHTED PERFORMANCE vs QUESTION LENGTH")
print("="*50)
...
# Save the figure
plt.savefig('/Users/tapetrova/PycharmProjects/Car_paper_v2/pythonProject/dataset_creation/analysis/weighted_performance_vs_question_length.png',
            dpi=300, bbox_inches='tight')
plt.close()

print("\nCreated weighted_performance_vs_question_length.png")
print("\nDetailed statistics (WFAS):")
print("-" * 50)
