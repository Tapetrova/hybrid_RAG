    ...
    if qid in question_lengths:
        ...
        for min_len, max_len, bin_label in length_bins:
            if min_len <= q_length < max_len:
                # Calculate FAS for each method (FAS = 1 - HR)
                for method in methods:
                    if method in result.get('metrics', {}):
                        metrics = result['metrics'][method]
                        # FAS = supported / total_claims * 100
                        if metrics.get('total_claims', 0) > 0:
                            fas = (metrics.get('supported', 0) / metrics['total_claims']) * 100
...
# Calculate correlation coefficients but print them instead of showing on plot
print("\n" + "="*50)
print("KEY INSIGHTS FROM THE GRAPH:")
print("="*50)
print("\n1. Performance degrades with longer questions for all methods")
print("\n2. Correlation analysis with question length:")
print("-" * 40)
