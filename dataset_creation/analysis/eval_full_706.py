#!/usr/bin/env python3
"""
ПОЛНЫЙ ЭКСПЕРИМЕНТ: Оценка всех 4 методов на ВСЕХ 706 вопросах
Только РЕАЛЬНЫЕ данные! Никаких моков!
"""
...
class FullEvaluation706:
    def __init__(self):
        ...
        print(f"📚 Загружен ПОЛНЫЙ датасет: {len(self.dataset['questions'])} вопросов")
        
        # Файлы для сохранения
        self.output_file = f"eval_FULL_706_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        self.checkpoint_file = "eval_FULL_checkpoint.json"
    ...
    def get_tavily_context(self, query: str, search_type: str = "basic") -> list:
        """Получаем РЕАЛЬНЫЙ контекст через Tavily API"""
        
        url = "https://api.tavily.com/search"
        
        # Модифицируем запрос для разных типов поиска
...
Focus on cause-effect relationships:"""
    ...
    def test_hybrid_ahs(self, question: str, category: str) -> dict:
        ...
        try:
            ...
            time.sleep(0.5)  # Небольшая пауза между API вызовами
            graph_contexts = self.get_tavily_context(question, "causal")
            
            # Комбинируем контексты по категории
...
Comprehensive answer using all information:"""
    ...
    def run_full_evaluation(self):
        ...
        print("="*80)
        print(f"🚀 ПОЛНЫЙ ЭКСПЕРИМЕНТ: ОЦЕНКА ВСЕХ {len(self.dataset['questions'])} ВОПРОСОВ")
        print("="*80)
        print("⏱️ Ожидаемое время: 2-3 часа")
        print("💡 Checkpoint сохраняется каждые 10 вопросов")
        print()
        ...
        print(f"📊 К обработке: {len(questions_to_process)} вопросов")
        ...
        for i, q_data in enumerate(questions_to_process, 1):
            try:
                ...
                # Сохраняем checkpoint каждые 10 вопросов
                if len(self.results) % 10 == 0:
                    self.save_checkpoint()
                    print(f"   💾 Checkpoint сохранён ({len(self.results)} вопросов)")
                
                # Показываем прогресс каждые 50 вопросов
                if len(self.results) % 50 == 0:
                    elapsed = (datetime.now() - self.start_time).total_seconds()
                    avg_time = elapsed / len(self.results)
                    remaining = (len(self.dataset['questions']) - len(self.results)) * avg_time
                    print(f"\n⏱️ ПРОГРЕСС: {len(self.results)}/{len(self.dataset['questions'])}")
                    print(f"   Среднее время на вопрос: {avg_time:.1f} сек")
                    print(f"   Осталось примерно: {remaining/60:.1f} мин")
                    print()
                
                # Пауза между вопросами для API
                time.sleep(1)
    ...
    def save_final_results(self):
        ...
        print("\n" + "="*80)
        print("🎉 ПОЛНЫЙ ЭКСПЕРИМЕНТ ЗАВЕРШЁН!")
        print("="*80)
    ...
    def calculate_statistics(self) -> dict:
        ...
        stats = {
            ...
            "dataset_total": len(self.dataset['questions']),
            "completion_rate": f"{len(self.results)/len(self.dataset['questions'])*100:.1f}%",
            "timestamp": datetime.now().isoformat(),
            "by_category": {},
            "by_mode": {
                "base_llm": {"success": 0, "errors": 0},
                "vector_rag": {"success": 0, "errors": 0, "total_context": 0},
                "graph_rag": {"success": 0, "errors": 0, "total_context": 0},
                "hybrid_ahs": {"success": 0, "errors": 0, "total_context": 0}
            }
        }
            ...
            for mode in ['base_llm', 'vector_rag', 'graph_rag', 'hybrid_ahs']:
                if mode in r:
                    if r[mode].get('success', False):
                        ...
                        if mode != 'base_llm':
                            stats['by_mode'][mode]['total_context'] += r[mode].get('context_size', 0)
                    else:
...
def main():
    print("🚀 ЗАПУСК ПОЛНОГО ЭКСПЕРИМЕНТА НА 706 ВОПРОСАХ")
    print("⚠️ Это займёт примерно 2-3 часа")
    print("✅ Используются ТОЛЬКО РЕАЛЬНЫЕ API (Tavily + OpenAI)")
    print()
    
    evaluator = FullEvaluation706()
